				...
				<v-row>
					...
					<v-col cols="12" sm="6">
						<v-sheet max-width="500" class="mx-auto">
							...
							<v-divider
								class="border-opacity-100 mt-6 mb-5"
								color="#222"
							></v-divider>
							<div class="d-flex justify-end">
								<div>
									<v-btn
										...
										variant="plain"
										:icon="
											isActive ? 'fa-regular fa-heart' : 'fa-solid fa-heart'
										"
									></v-btn>
									<v-tooltip
										activator="parent"
									...
									<p
										v-for="item in data"
										:key="item.title"
										class="text-caption ma-0 mt-n2 mr-1"
									>
						...
						<v-sheet max-width="760" class="mx-auto text-left my-4">
							...
							<v-container>
								<v-row>
									...
									<v-col cols="auto">
										<p v-if="!HaveCode" class="text-body-2 ml-n3 mt-1">
											{{ UserInfoName }}さん
										</p>
									</v-col>
								</v-row>
							...
							<v-col>
								<v-form ref="form" @submit.prevent="post">
									<v-textarea
										...
										:error-messages="
											v$.message_comment.$errors.map((e: any) => e.$message)
										"
										rows="3"
										auto-grow
										label="コメントはこちらへ"
