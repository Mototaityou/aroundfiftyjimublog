              ...
              >
                <v-hover v-slot="{ isHovering, props }">
                  <v-card
                    ...
                    v-bind="props"
                  >
                    <v-img cover max-height="150" :src="item.image" :alt="item.title"></v-img>
                    <h2 id="blogtitle" class="text-left ml-4 mt-4 mr-4">
                      {{ item.title }}
                    </h2>
                    <p class="text-left text-caption font-weight-bold ml-4">
                      {{ dayjs(item.created_at).format('YYYY.MM.DD') }}
                    </p>
                    <p class="text-left text-body-2 ml-4 mb-4 mr-4">
                      {{ item.subtitle }}
                    </p>
                  </v-card>
                </v-hover>
