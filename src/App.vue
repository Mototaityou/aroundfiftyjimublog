...
function createTitleDesc(routeInstance: RouteLocation) {
	...
	if (routeInstance.path) {
		const isAlternate = routeInstance.path;
		const setAlternate = 'https://aroundfiftyjimu.com' + routeInstance.path;
		console.log(isAlternate);
		console.log(setAlternate);
		if (isAlternate.match(/\/mobile/)) {
			console.log(isAlternate);
			document
			...
			document
				.querySelector("link[name='isAlternate']")
				?.setAttribute('href', setAlternate);
		} else {
			console.log(isAlternate);
			document
				.querySelector("link[name='isAlternate']")
